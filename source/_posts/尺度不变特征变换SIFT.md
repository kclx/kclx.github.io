...
尺度不变特征变换（Scale-Invariant Feature Transform，SIFT）是一种用于计算机视觉和图像处理的算法。它是由David
Lowe在1999年提出的，并成为一种广泛应用的特征提取和匹配方法。
...
3. 对于找到的极值点，根据其响应值（如像素值或梯度大小）进行阈值处理和边界消除。通常，会设置一个阈值来筛选出具有较大响应值的极值点，同时通过非极大值抑制（non-maximum
   suppression）来消除邻域内的重复极值点。
